...
/* harmony import */ var _test_form__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ./test/form */ "./src/js/test/form.js");
/* harmony import */ var _test_form__WEBPACK_IMPORTED_MODULE_9___default = /*#__PURE__*/__webpack_require__.n(_test_form__WEBPACK_IMPORTED_MODULE_9__);
/* harmony import */ var _test_indicator__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./test/indicator */ "./src/js/test/indicator.js");
/* harmony import */ var _test_indicator__WEBPACK_IMPORTED_MODULE_10___default = /*#__PURE__*/__webpack_require__.n(_test_indicator__WEBPACK_IMPORTED_MODULE_10__);
...
var inputValue = document.querySelectorAll('[type="text"]');

var getEventListener = function getEventListener(event) {
  formContainer.addEventListener(event, function (e) {
    var target = e.target;
    var allInputs = target.closest('.js-all-inputs');
    var currentLabel = allInputs.querySelector('.js-input-label');
...
document.addEventListener("DOMContentLoaded", function () {
  inputValue.forEach(function (inputLabel) {
    if (inputLabel.value) {
 ...
 * Индикатор JS скилла
 * */
var indicatorArrow = document.querySelector('.js-indicator-arrow');
var checkboxContent = document.querySelector('.js-checkbox-content');
var checkboxMark = document.querySelectorAll('.js-checkbox-mark');
var indicatedSkills = ['vanillajs', 'gulp', 'angular', 'jquery'];
var oneIndicatorDivision = 180 / indicatedSkills.length;
var numberIndicatorElements = 1000 / indicatedSkills.length;
var startingPoint = 270;
var startKnowledgeCounter = 0;

function easeCounterChanged() {
  var roundingDivision = Math.ceil(numberIndicatorElements);
...
function addValueToIndicator(action, element) {
  var levelLowerText = element.nextElementSibling.textContent.toLowerCase();
  var knowledgeCounter = document.querySelector('.js-knowledge-counter');
  indicatedSkills.forEach(function (item) {
    if (levelLowerText === item) {
      var roundingDivision = Math.ceil(numberIndicatorElements);
      var renderKnowledgeCounter = action === 'add' ? startKnowledgeCounter += roundingDivision : startKnowledgeCounter -= roundingDivision;
      var roundingAsInteger = Math.round(parseInt(renderKnowledgeCounter, 10) / 10) * 10;
      ...
      easeCounterChanged();
      indicatorArrow.style.transform = "translateX(-50%) rotate(".concat(startingPoint, "deg)");
    }
  });
}
...
function listenerChecked(e) {
  var target = e.target;
  var targetChecked = target.classList.contains('js-checkbox-mark');
  var targetInputLabel = target.classList.contains('js-checkbox-label');

  if (!targetChecked && !targetInputLabel) {
    return;
  }

  var currentCheckbox = target.closest('.js-checkbox');
  var currentDataset = currentCheckbox.dataset.skill;
  checkboxMark.forEach(function (mark) {
...
checkboxContent.addEventListener('keydown', function (e) {
  if (e.code === 'Space') {
    listenerChecked(e);
  }
});
